...
export default function DashboardLayout({
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex">
            ...
            <aside
                className={cn(
                    "bg-bnp-emerald-dark text-white transition-all duration-300 flex flex-col fixed inset-y-0 left-0 z-50",
                    sidebarOpen ? "w-64" : "w-16"
                )}
            >
                <div className="h-16 flex items-center px-4 border-b border-green-800">
                    <div className="bg-white text-bnp-emerald font-bold p-1 rounded mr-3">FH</div>
                ...
                <div className="p-4 border-t border-green-800 space-y-4">
                    ...
                    <div className="flex items-center justify-center">
                        <div className="flex items-center bg-white rounded-full p-1 border border-green-800/30 shadow-sm">
                            <button className="flex items-center px-3 py-1 bg-bnp-emerald text-white text-xs font-bold rounded-full transition-all">
                                <span className="mr-1">✓</span> FR
                            </button>
                            <button className="px-3 py-1 text-gray-500 text-xs font-bold hover:text-bnp-emerald transition-colors">
                                EN
                            </button>
                        </div>
                    </div>
...
function NavLink({ icon, label, isOpen, active = false }: { icon: any, label: string, isOpen: boolean, active?: boolean }) {
    return (
        <div className={cn(
            ...
            active
                ? "border-bnp-emerald bg-green-900/30 text-white"
